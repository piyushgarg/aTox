...
JNIEXPORT jlong JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavNew(JNIEnv *env, jobject thiz, jlong toxPtr) {
    if (!toxPtr) return 0;
    TOXAV_ERR_NEW err;
...
JNIEXPORT void JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavKill(JNIEnv *env, jobject thiz, jlong avPtr) {
...
JNIEXPORT void JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavIterate(JNIEnv *env, jobject thiz, jlong avPtr, jobject listener) {
    ToxAV *av = reinterpret_cast<ToxAV*>(avPtr);
    if (av) {
...
JNIEXPORT jint JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavIterationInterval(JNIEnv *env, jobject thiz, jlong avPtr) {
    return avPtr ? toxav_iteration_interval(reinterpret_cast<ToxAV*>(avPtr)) : 200;
}

JNIEXPORT jboolean JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavCall(JNIEnv *env, jobject thiz, jlong avPtr, jint friendNumber, jint audioBitrate, jint videoBitrate) {
    return toxav_call(reinterpret_cast<ToxAV*>(avPtr), friendNumber, audioBitrate, videoBitrate, nullptr);
}

JNIEXPORT jboolean JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavAnswer(JNIEnv *env, jobject thiz, jlong avPtr, jint friendNumber, jint audioBitrate, jint videoBitrate) {
    return toxav_answer(reinterpret_cast<ToxAV*>(avPtr), friendNumber, audioBitrate, videoBitrate, nullptr);
}

JNIEXPORT jboolean JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavCallControl(JNIEnv *env, jobject thiz, jlong avPtr, jint friendNumber, jint control) {
    return toxav_call_control(reinterpret_cast<ToxAV*>(avPtr), friendNumber, (TOXAV_CALL_CONTROL)control, nullptr);
}

JNIEXPORT jboolean JNICALL Java_ltd_evilcorp_domain_tox_NativeToxAv_toxavAudioSendFrame(JNIEnv *env, jobject thiz, jlong avPtr, jint friendNumber, jshortArray pcm, jint sampleCount, jint channels, jint samplingRate) {
...
bool register_toxav_methods(JNIEnv* env) {
    jclass cls = env->FindClass("ltd/evilcorp/domain/tox/ToxAvEventListener");
    if (!cls) return false;
